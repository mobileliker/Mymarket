			...
			{!! Form::model($category,['route'=>['wpanel.category.update',$category],'method'=>'PUT','class'=>'form-horizontal','role'=>'form','enctype'=>'multipart/form-data']) !!}
				...
				<div class="row form-group">
					...
					<label class="col-md-2 control-label">分类图标(导航):</label>
						...
						@if(!empty($category->image_nav))
							<img src="{{$category->image_nav}}" alt="点击" class="thumbnail" onclick="$('input[id=pic_nav]').click();" width="40px" height="40px" style="cursor:pointer">
						@else
							<img src="[['/img/no-image.jpg']]" alt="点击" class="thumbnail" onclick="$('input[id=pic_nav]').click();" width="40px" height="40px" style="cursor:pointer">
						@endif
						</div>
				</div>
